...
When a user supplies a repository-path or complete/partial external-link mention rather than an already
readable path, load `$doctidex-git-mentions` once and return here with its candidate or diagnostic. That
Skill owns context completion and `link-parse`; do not search for a suffix or invoke the parser directly
to recreate its result. Continue native reading only from an available returned `working_path`.
...
symlink cannot be entered because its target is missing or inaccessible, load
`$doctidex-git-mentions` with the symlink itself and return here with its exact parse result.
...
creates managed state inside installed content. After optional Maintenance finishes, rerun
`$doctidex-git-mentions` with the same path and continue from the outer `working_path`. If Maintenance
was already loaded, resume it without reopening either Skill.
