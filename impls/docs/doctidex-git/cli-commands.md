...
| `inspect [PATH]` | PATH，默认 cwd | PATH。 |
| `resolve` | cwd | `INTERNAL_PATH` 相对于所选根。 |
| 所有 `mount` | cwd | 所选宿主根。 |
| 所有 `maintenance` | cwd | scope 的 PATH 只用于 scope 分类，不改变宿主根选择。 |
...
返回输入、规范化路径、link root、文件系统路径、是否跨 mount 和完整 mount 状态。
mount 未准备时命令本身仍 `status: ok`，通过 `result` 和 mount 的 `next_action` 提示
prepare；resolve 不自动恢复 mount。
...
blocked。每项 state 为 `ready` 或 `has_changes`。
...
和决定 Git 动作。clean 时删除 worktree 和 state record。
