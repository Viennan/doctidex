...
class PartialDisclosureTests(unittest.TestCase):
    ...
    def test_whero_submodule_update_uses_inner_git_structure(self) -> None:
        ...
        self.assert_succeeded(content_update)
        self.assertIn(
            f'source_commit: "{content_commit}"',
            inner_status.read_text(encoding="utf-8"),
        )
        ...
        self.assertIn(f'source_commit: "{content_commit}"', retained_status)
    ...
    def test_forward_content_only_change_is_accepted(self) -> None:
        repository = self.root / "repository"
        source = repository / "whero-wiki"
        self.create_wiki(source)
        ...
        self.assert_succeeded(result)
        self.assertIn("Git source advanced", result.stdout)
        self.assertIn("no structural change", result.stdout)
