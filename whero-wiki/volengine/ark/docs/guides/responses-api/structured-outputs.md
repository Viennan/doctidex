...
<Tabs>
<Tab zoneid="xCnDXqxv2r" title="Curl">
...
<Tab zoneid="oZSqAqQS2D" title="Python">
...
<Tab zoneid="tfUqQGhjXo" title="Go">
...
<Tab zoneid="pnf4mXp4b6" title="Java">
...
<Tab zoneid="aZD0I0H9uJ" title="OpenAI SDK">
...
<Tabs>
<Tab zoneid="a1UT32APsJ" title="Curl">
<TabTitle>Curl</TabTitle>
...
<Tab zoneid="FAD7waPrqL" title="OpenAI SDK">
<TabTitle>OpenAI SDK</TabTitle>
...
|严格模式 |支持<br><br>通过设置 **strict** 为 `true` 生效。 |不涉及 |
|配置方式 |```JSON```<br>```"text": {```<br>```    "format": {```<br>```        "type": "json_schema",```<br>```        "name": "my_schema",```<br>```        "strict": true,```<br>```        "schema": {```<br>```            ...```<br>```        }```<br>```    }```<br>```}```<br> |```JSON```<br>```"text": {```<br>```  "format": {```<br>```    "type": "json_object"```<br>```  }```<br>```}```<br> |
