...
<div data-tips="true" data-tips-type="tip">仅支持查询最近 7 天的任务记录，时间区间为 [T\-7天, T)，其中 T 为请求发起时刻的 UTC 时间戳（精确到秒）。注意：视频 URL 有效期为 24 小时，请及时下载或转存。</div>
...
说明：[创建视频生成任务](/docs/82379/1520757){target="_self"} 时设置 `"return_last_frame": true` 时，会返回参数。
```
