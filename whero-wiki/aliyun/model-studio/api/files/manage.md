...
| **字段** | **类型** | **传参方式** | **必选** | **描述** |
| --- | --- | --- | --- | --- |
| file\\_id | String | path | 是   | 文件ID，可通过[上传文件](https://help.aliyun.com/zh/model-studio/upload-file-api#t6612629.html)或列举文件接口获取 |
...
| data.files | Array | 文件列表，元素为文件对象，属性详见文件对象小节 |
| request\\_id | String | 本次请求的系统唯一码 |
...
文件对象表示已上传到百炼平台的文件及其属性信息。通过[上传文件](https://help.aliyun.com/zh/model-studio/upload-file-api#t6612629.html)接口创建，可通过列举文件和查询文件详情接口获取。
...
| md5 | String | 文件的md5 |
| url | String | 文件下载链接 |
| gmt\\_create | Date | 文件上传时间 |
| id  | String | 文件的内部ID |
| region | String | 文件所在地域 |
| user\\_id | String | 用户ID |
| api\\_key\\_id | String | API Key ID |
| purpose | String | 文件用途 |