...
type CommandExitStatus = Callable[[CommandPayload], int]
...
def _default_exit_status(payload: CommandPayload) -> int:
    return 0 if payload["status"] == "ok" else 2


def _validate_exit_status(payload: CommandPayload) -> int:
    if payload["status"] != "ok":
        return 2
    return 0 if payload["valid"] else 1


def _command_result[**P](
    *,
    ...
    def decorate(
        ...
        def run(invocation: ParsedInvocation, /, *args: P.args, **kwargs: P.kwargs) -> int:
            ...
            try:
                ...
                payload = _store_or_model_failure(operation, exc)
            print(_json(payload))
