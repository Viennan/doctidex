...
`hook` installs and runs Git hooks that keep the untracked runtime work model consistent across branch switches.
...
`hook install` writes the supported hook scripts under the repository's Git hooks directory. It uses the resolved
`doctidex-git` command path in the script, so hook execution does not depend on `PATH`. The command is idempotent.
...
`hook install` and `hook post-checkout` are forbidden inside a managed Installation.
