...
|---|---|---|---|
| [Tree/configuration](../../architecture/models/doctidex-tree-and-configuration.md) | `DoctidexDocument`, `MarkdownLink`, `IndexInfo`, `_Validator`；annotation/edge 是局部 helper facts | [Protocol interpreter](components/protocol-interpreter.md) | `test_boundary_unsafe_annotation_and_reachability`, `test_index_log_and_atomic_rules`, `test_reference_link_annotation_is_associated_with_the_link`, `test_scoped_validation_does_not_read_unrelated_subtree`。 |
...
| [Link parse](../../architecture/system/external-workflows.md#4-link-parse) | Read Skill / CLI | current + portable mapping resolver | `test_portable_broken_link_dependency_can_be_flattened`, `test_link_retry_rejects_a_changed_symlink_target`。 |
...
| [Cache clean](../../architecture/system/worktree-and-cache-workflows.md#4-cache-clean) | human/program CLI | `CacheService.clean` + Git registration classification | `test_cache_cleanup_preserves_active_then_removes_eligible`, `test_cache_cleanup_accepts_prunable_registration`。 |
...
mapping、native-current maintenance、isolated worktree、single-source cache cleanup、agent routing、
